...
# 输入和输出文件路径
input_file = 'train.json'  # 原始数据集文件
output_file = 'train0.json'  # 转换后的数据集文件

# 1. 读取原始数据集
with open(input_file, 'r', encoding='utf-8') as f:
    original_data = json.load(f)  # 假设原始数据是JSON数组

# 2. 转换数据格式，添加固定内容的instruction项
converted_data = []
for item in original_data:
    # 创建符合Llama-Factory SFT格式的条目，添加固定内容的instruction项
    converted_item = {
    ...
    "现在处理文本：",  # 添加固定指令文本
        ...
        "output": item["output"]   # 输出保持不变
    }
    converted_data.append(converted_item)

# 3. 保存为Llama-Factory可用的格式
with open(output_file, 'w', encoding='utf-8') as f:
    json.dump(converted_data, f, indent=2, ensure_ascii=False)

print(f"数据集转换完成！结果已保存至 {output_file}")
